...
function create_jo_p1_from_so(frm) {
	frappe.call({
		method: "iib.iib.doctype.job_order_corrugator.job_order_corrugator.get_so_items_for_corrugator",
...
function build_jo_p1(frm, rows) {
	frappe.model.with_doctype("Job Order Corrugator", () => {
		const jo = frappe.model.get_new_doc("Job Order Corrugator");
		...
		frappe.set_route("Form", "Job Order Corrugator", jo.name);
	});
}

function create_jo_p2_from_so(frm) {
	frappe.call({
		method: "iib.iib.doctype.job_order_converting.job_order_converting.get_sales_orders_for_jo",
...
function build_jo(frm, open_rows, production_item) {
	frappe.model.with_doctype("Job Order Converting", () => {
		const jo = frappe.model.get_new_doc("Job Order Converting");
		jo.company = frm.doc.company;
		...
		frappe.set_route("Form", "Job Order Converting", jo.name);
	});
}
