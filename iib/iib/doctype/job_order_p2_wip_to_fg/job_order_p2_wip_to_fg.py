...
class JobOrderP2WIPtoFG(StockController):
	"""WIP → Finished Goods transfer for Job Order P2.

	Inherits StockController so that SL entries carry
	voucher_type = "Job Order P2 WIP to FG" instead of "Stock Entry".
	"""
	...
	def set_defaults(self):
		...
		# Warehouses come from the linked Job Order P2
		if self.job_order_p2 and (not self.source_warehouse or not self.target_warehouse):
	...
	def _cascade_to_job_order(self):
		"""Recompute produced_qty and status on the linked Job Order P2."""
