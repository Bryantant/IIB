...
class JobOrderConvertingRMtoWIP(StockController):
	"""Raw Material → WIP transfer for Job Order Converting.

	Inherits StockController so that SL entries carry
	voucher_type = "Job Order Converting RM to WIP" instead of "Stock Entry".
	"""
	...
	def set_defaults(self):
		...
		if not self.source_warehouse:
			self.source_warehouse = settings.default_raw_material_warehouse
		if not self.target_warehouse:
			self.target_warehouse = settings.default_wip_warehouse
	...
	def update_stock_ledger(self):
		...
		for d in self.get("items"):
			if not d.item_code:
				continue
			# Deduct from source (Raw Material warehouse)
			sl_entries.append(
				self.get_sl_entries(
					d,
					{
	...
	# -------------------------------------------------------------------------
	# GL Entries (Dr target warehouse account, Cr source warehouse account)
	# -------------------------------------------------------------------------
	...
	def get_gl_entries(
		...
		for d in self.get("items"):
			...
			if source_account == target_account:
				# Same GL account — internal movement, no net GL impact
				continue

			remarks = _("RM to WIP Transfer: {0}").format(self.name)

			# Dr WIP (target) — WIP asset increases
			gl_entries.append(
				self.get_gl_dict(
					{
			...
			# Cr Raw Material (source) — RM asset decreases
			gl_entries.append(
				self.get_gl_dict(
					{
